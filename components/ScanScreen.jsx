...
function ScanScreen({ sky, onComplete, onCancel, mode }) {
  ...
  var [error, setError] = React.useState(null); // {kind, headline, body, hint}
  ...
  // Simulate scan + chance of an error every tick
  React.useEffect(function() {
    if (phase !== 'scanning') return;
    var t = setInterval(function() {
      // ~10% per second after first 3s, but never on a retry > 1
      if (elapsed > 3 && retryCount === 0 && Math.random() < 0.10) {
        var kinds = [
          { kind: 'noFinger', headline: 'I can’t see your finger.', body: 'Cover both the lens and the flash with your fingertip.', hint: 'Try again' },
          { kind: 'lowLight', headline: 'Not enough light.', body: 'Move to a brighter spot, or hold the flash closer to your fingertip.', hint: 'Try again' },
          { kind: 'noisy', headline: 'Too much motion.', body: 'Rest your hand on a flat surface and breathe steady.', hint: 'Try again' },
          { kind: 'lostSignal', headline: 'I lost your pulse.', body: 'Soften your finger — you may be pressing too hard.', hint: 'Try again' }
        ];
        var pick = kinds[Math.floor(Math.random() * kinds.length)];
        setError(pick); setPhase('error'); clearInterval(t); return;
      }
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        ...
        {/* ERROR STATE — friendly recovery */}
        {phase === 'error' && error && (
          <div style={{
            textAlign: 'center', padding: '0 32px', animation: 'varaFadeUp 0.5s ease',
            display: 'flex', flexDirection: 'column', alignItems: 'center'
          }}>
            <div style={{
              width: 90, height: 90, borderRadius: '50%', marginBottom: 28,
              background: 'radial-gradient(circle, rgba(196,106,90,0.22) 0%, transparent 70%)',
              border: '0.5px solid rgba(196,106,90,0.4)',
              display: 'flex', alignItems: 'center', justifyContent: 'center'
            }}>
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="12" r="9" stroke="#C46A5A" strokeWidth="1.4" />
                <path d="M12 8v4" stroke="#C46A5A" strokeWidth="1.8" strokeLinecap="round" />
                <circle cx="12" cy="15.5" r="0.9" fill="#C46A5A" />
              </svg>
            </div>
            <div style={{
              fontSize: 22, fontWeight: 300, color: sky.txt,
              letterSpacing: '-0.01em', lineHeight: 1.22, marginBottom: 12, textWrap: 'pretty'
            }}>{error.headline}</div>
            <p style={{
              fontSize: 15, fontWeight: 300, color: sky.sub,
              lineHeight: 1.6, maxWidth: 280, margin: '0 auto 32px'
            }}>{error.body}</p>
            <button onClick={function () {
              setError(null); setRetryCount(retryCount + 1);
              setElapsed(0); setSi(null);
              setPhase('waiting');
            }} style={{
              padding: '14px 36px', borderRadius: 100,
              background: sky.fp + '20', border: '0.5px solid ' + sky.fp + '60',
              cursor: 'pointer', fontFamily: 'var(--vara-sans)',
              fontSize: 14, fontWeight: 500, letterSpacing: '0.18em',
              textTransform: 'uppercase', color: sky.fp, marginBottom: 12
            }}>{error.hint}</button>
            <button onClick={onCancel} style={{
              background: 'transparent', border: 'none', cursor: 'pointer',
              fontFamily: 'var(--vara-sans)', padding: '6px 12px',
              fontSize: 12.5, fontWeight: 400, letterSpacing: '0.14em',
        ...
        {phase === 'moment' && (
          <div style={{ textAlign: 'center', padding: '0 32px', animation: 'varaFadeUp 0.8s ease', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <p style={{
              fontSize: 16, fontWeight: 300, color: sky.sub,
            ...
            <button onClick={function() { setPhase('waiting'); }} style={{
              ...
              <div style={{
                ...
                fontFamily: 'var(--vara-sans)',
                fontSize: 12.5, fontWeight: 500, letterSpacing: '0.28em',
        ...
        {/* progress ring (hidden during the moment + error screens) */}
        {phase !== 'moment' && phase !== 'error' && <div style={{ position: 'relative', width: 260, height: 260 }}>
          ...
          <div style={{
            ...
            {phase === 'waiting' && (
              <button onClick={function() { setPhase('scanning'); }} style={{
                ...
                <div style={{
                  marginTop: 12, fontSize: 11.5, fontWeight: 500, letterSpacing: '0.28em',
                  textTransform: 'uppercase', color: sky.sub,
                }}>Begin scan</div>
              </button>
            )}
            ...
            {phase === 'done' && (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{
                  fontSize: 12.5, fontWeight: 500, letterSpacing: '0.24em',
      ...
      <div style={{
        ...
        {phase === 'done' && (
          <div style={{
            fontSize: 16, fontWeight: 400, color: sky.txt, textAlign: 'center',
