...
function SettingsScreen({ sky, onBack, onNavigate }) {
  var isDark = sky.dark;
  ...
  try {
    ...
    var raw = localStorage.getItem('suna.settings');
    if (raw) saved = JSON.parse(raw) || {};
  } catch (e) { initialName = 'Eva'; initialEmail = 'eva@suna.app'; saved = {}; }

  var [name, setName] = React.useState(initialName);
  var [email] = React.useState(initialEmail);
  var [cycleOn, setCycleOn] = React.useState(saved.cycleOn !== false);
  var [cycleStart, setCycleStart] = React.useState(saved.cycleStart || '2026-04-29');
  var [cycleLength, setCycleLength] = React.useState(saved.cycleLength || 28);
  var [wake, setWake] = React.useState(saved.wake || '07:30');
  var [reminderOn, setReminderOn] = React.useState(saved.reminderOn !== false);
  var [reminderTime, setReminderTime] = React.useState(saved.reminderTime || '08:30');
  var [theme, setTheme] = React.useState(saved.theme || (isDark ? 'dark' : 'light'));
  var [hapticsOn, setHapticsOn] = React.useState(saved.hapticsOn !== false);
  var [restoreState, setRestoreState] = React.useState('idle'); // idle | restoring | restored | none

  // Membership state — in real app comes from StoreKit/Play Billing
  var membership = {
    active: true,
    plan: saved.plan || 'annual',
    renewsOn: 'Sep 14, 2026',
    trialing: !localStorage.getItem('suna.trialEnded')
  };

  function persist() {
    try {
      if (name) localStorage.setItem('suna.userName', name.trim().split(' ')[0]);
      localStorage.setItem('suna.settings', JSON.stringify({
        cycleOn: cycleOn, cycleStart: cycleStart, cycleLength: cycleLength,
        wake: wake, reminderOn: reminderOn, reminderTime: reminderTime,
        theme: theme, hapticsOn: hapticsOn, plan: membership.plan
      }));
    } catch (e) {}
  }
  React.useEffect(persist, [name, cycleOn, cycleStart, cycleLength, wake, reminderOn, reminderTime, theme, hapticsOn]);

  function startRestore() {
    setRestoreState('restoring');
    setTimeout(function () {
      // mock: 60% chance restored, 40% none. In dev this is real StoreKit/Play.
      setRestoreState(Math.random() < 0.7 ? 'restored' : 'none');
    }, 1400);
  }
  ...
  function Row({ label, children, last }) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 15, fontWeight: 400, color: sky.txt, letterSpacing: '-0.005em' }}>{label}</div>
        <div style={{ fontSize: 15, fontWeight: 300, color: sky.sub }}>{children}</div>
      </div>
    );
  }
  ...
  function TimeInput({ value, onChange }) {
    return (
      <input type="time" value={value} onChange={function (e) { onChange(e.target.value); }} style={{
        background: 'transparent', border: 'none', color: sky.txt, fontSize: 15, fontWeight: 400,
    ...
    <div className="vara-scroll" data-dc-passthrough style={{
      ...
      <div style={{ position: 'relative', padding: '60px 24px 120px' }}>
        ...
        <button onClick={onBack} style={{
          ...
          fontSize: 12.5, fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase',
        ...
        <div style={{ fontSize: 14, fontWeight: 300, fontStyle: 'italic', color: sky.sub,
        ...
        <Section title="Account">
          ...
          <Row label="Email" last>
            <span style={{ fontFamily: 'var(--vara-mono), var(--vara-sans)', fontSize: 14 }}>{email}</span>
          </Row>
        </Section>

        {/* Membership */}
        <Section title="Membership">
          <div style={{ padding: '16px 16px', borderBottom: '0.5px solid ' + lineSoft }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: sky.fp, boxShadow: '0 0 8px ' + sky.fp + '90' }} />
                <span style={{ fontSize: 15, fontWeight: 500, color: sky.txt, letterSpacing: '-0.005em' }}>
                  suna {membership.active ? 'Premium' : 'Free'}
                </span>
              </div>
              <span style={{ fontSize: 11.5, fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase',
                color: sky.fp, padding: '4px 10px', borderRadius: 100,
                background: sky.fp + '15', border: '0.5px solid ' + sky.fp + '50' }}>
                {membership.plan === 'annual' ? 'Annual' : 'Monthly'}
              </span>
            </div>
            <div style={{ fontSize: 13, fontWeight: 300, color: sky.sub, letterSpacing: '0.02em', lineHeight: 1.5 }}>
              {membership.trialing ? 'Free trial · ' : ''}
              {membership.plan === 'annual' ? 'Renews ' : 'Bills '}{membership.renewsOn} at ${membership.plan === 'annual' ? '99/yr' : '12.99/mo'}
            </div>
          </div>
          <button onClick={function () { onNavigate && onNavigate('paywall'); }} style={rowBtn(sky, lineSoft)}>
            <span>Manage subscription</span><span style={{ color: sky.fp }}>{'\u203A'}</span>
          </button>
          <button onClick={startRestore} style={rowBtn(sky, lineSoft)}>
            <span>Restore purchase</span>
            <span style={{ color: sky.sub, fontFamily: 'var(--vara-mono), var(--vara-sans)', fontSize: 12.5 }}>
              {restoreState === 'restoring' ? '…' : restoreState === 'restored' ? 'restored ✓' : restoreState === 'none' ? 'no purchase' : ''}
            </span>
          </button>
          <button onClick={function () { onNavigate && onNavigate('paywall'); }} style={Object.assign({}, rowBtn(sky, lineSoft), { borderBottom: 'none' })}>
            <span>Change plan</span><span style={{ color: sky.sub, opacity: 0.8 }}>{'\u203A'}</span>
          </button>
        </Section>
        ...
        <Section title="Cycle Sync">
            ...
            <>
              <Row label="Last period started">
                <input type="date" value={cycleStart} onChange={function (e) { setCycleStart(e.target.value); }} style={{
                  background: 'transparent', border: 'none', color: sky.txt, fontSize: 15,
          ...
          <div style={{ padding: '12px 16px', borderTop: '0.5px solid ' + lineSoft }}>
            <p style={{ fontSize: 12.5, fontWeight: 300, fontStyle: 'italic', color: sky.sub,
              ...
              {['light', 'dark', 'auto'].map(function (t) {
                ...
                return (
                  <button key={t} onClick={function () { setTheme(t); }} style={{
                    ...
                    fontFamily: 'var(--vara-sans)', fontSize: 12.5, fontWeight: 500,
        ...
        <Section title="Privacy & Data">
          <Row label="On-device only">
            <span style={{ fontSize: 12.5, fontWeight: 500, letterSpacing: '0.16em', textTransform: 'uppercase',
              color: sky.fp }}>Always</span>
          </Row>
          <button onClick={function () { onNavigate && onNavigate('export'); }} style={rowBtn(sky, lineSoft)}>
            <span>Export my data</span><span style={{ color: sky.fp }}>{'\u203A'}</span>
          </button>
          <button onClick={function () { onNavigate && onNavigate('deleteConfirm'); }} style={Object.assign({}, rowBtn(sky, lineSoft), { borderBottom: 'none' })}>
            <span style={{ color: '#C46A5A' }}>Delete account</span>
            <span style={{ color: '#C46A5A', opacity: 0.7 }}>{'\u203A'}</span>
          </button>
        </Section>

        {/* version */}
        <div style={{ textAlign: 'center', fontSize: 11.5, fontWeight: 400, color: sky.sub,
...
function miniBtn(sky) {
  return {
    ...
    background: 'transparent', color: sky.txt, fontSize: 16, lineHeight: 1, cursor: 'pointer',
