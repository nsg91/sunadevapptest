...
function SansEyebrow({ children, color, style }) {
  return React.createElement('div', { style: Object.assign({
      fontFamily: 'var(--vara-sans)', fontSize: 11.5, fontWeight: 400,
...
function SunaMono({ children, color, style }) {
  return React.createElement('span', { style: Object.assign({
      fontFamily: 'var(--vara-mono), var(--vara-sans)', fontSize: 12.5, fontWeight: 500, color: color
    }, style || {}) }, children);
}
...
function SunaTabBar({ sky, isDark, onNavigate }) {
  ...
  const tl = (a) => ({ fontSize: 11.5, fontWeight: a ? 500 : 400,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: 11.5, fontWeight: 500, letterSpacing: '0.42em',
      ...
      <div style={{ padding: '24px 22px 20px' }}>
        ...
        <div style={{
          ...
          <button onClick={handleShare} style={{
            ...
            <span style={{
              fontSize: 11.5, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: sky.sub
            }}>Share</span>
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: 'var(--vara-mono), var(--vara-sans)', fontSize: 11.5, fontWeight: 500,
        ...
        <button onClick={onClose} style={{
          ...
          color: 'rgba(255,255,255,0.85)', cursor: 'pointer', fontSize: 16, lineHeight: 1
        }}>{'\u00D7'}</button>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: 16, fontWeight: 400, letterSpacing: '0.6em', textTransform: 'lowercase',
        ...
        <div style={{
          ...
          <div style={{
            marginTop: 22, fontSize: 12.5, fontWeight: 500,
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div style={{
        ...
        <div style={{
          fontFamily: 'var(--vara-mono), var(--vara-sans)',
          fontSize: 11.5, fontWeight: 500, color: sky.fp, letterSpacing: '0.18em'
        }}>{f.tag}</div>
        <div style={{
        ...
        <div style={{
          ...
          fontStyle: 'italic', fontSize: 15, fontWeight: 300, color: sky.fp,
          marginTop: 12, letterSpacing: '0.005em', lineHeight: 1.45
        }}>{f.pull}</div>
        <div style={{
          fontFamily: 'var(--vara-sans)',
          fontSize: 14, fontWeight: 300, color: sky.sub, marginTop: 16, lineHeight: 1.55
        }}>{f.body}</div>
        <div style={{
          fontFamily: 'var(--vara-mono), var(--vara-sans)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14 }}>
        ...
        <button onClick={shuffle} style={{
          ...
          fontFamily: 'var(--vara-sans)',
          fontSize: 12.5, fontWeight: 400, letterSpacing: '0.08em'
        }}>
...
function HomeV2({ sky, onOpenPractice, onNavigate }) {
  ...
  const recs = chip ? (window.getChipPractices ? window.getChipPractices(chip.label) : []) : [];
    ...
    <div style={{
      ...
      <div ref={scrollRef} className="vara-scroll" data-dc-passthrough style={{
        ...
        <div style={{ position: 'relative', height: 780, overflow: 'hidden' }}>
          ...
          }} data-comment-anchor="2b97eec87e-div-473-11" />
          {/* grain */}
          <div style={{
          ...
          <div style={{ position: 'relative', padding: '60px 28px 0' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <SunaMono color={sky.txt} style={{ letterSpacing: '0.42em', fontSize: 14 }}>suna</SunaMono>
              <button onClick={function () {if (onNavigate) onNavigate('profile');}} style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 14, fontWeight: 400, color: sky.txt,
                fontFamily: 'var(--vara-sans)'
              }}>E</button>
            </div>
            ...
            <div style={{ marginTop: 72, textAlign: 'center' }}>
              <SansEyebrow color={sky.sub} style={{ marginBottom: 14 }}>{(function(){
                var bits = [sky.phase];
                try { bits.push(localStorage.getItem('suna.userName') || 'Eva'); } catch(e) { bits.push('Eva'); }
                try {
                  var raw = localStorage.getItem('suna.settings');
                  if (raw) {
                    var s = JSON.parse(raw);
                    if (s && s.cycleOn !== false && s.cycleStart) {
                      var start = new Date(s.cycleStart);
                      var len = s.cycleLength || 28;
                      var diff = Math.floor((Date.now() - start.getTime()) / 86400000);
                      if (diff >= 0) {
                        var day = (diff % len) + 1;
                        var phase = day <= 5 ? 'menstrual' :
                                    day <= Math.round(len * 0.43) ? 'follicular' :
                                    day <= Math.round(len * 0.54) ? 'ovulation' :
                                    day <= Math.round(len * 0.82) ? 'luteal' :
                                    'late luteal';
                        bits.push('day ' + day + ' \u00b7 ' + phase);
                      }
                    }
                  }
                } catch(e) {}
            ...
            <div style={{ marginTop: 64, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              <div style={{
                marginTop: 26, textAlign: 'center', fontSize: 15, fontWeight: 300,
        ...
        <div style={{ position: 'relative', padding: '0 0 40px', marginTop: -40 }}>
          ...
          <div style={{ padding: '0 24px' }}>
              ...
              {STATE_CHIPS.map(function (c, i) {
                ...
                return (
                  <button key={c.label} onClick={function () {setChipIdx(active ? null : i);}} style={{
                    ...
                    <div style={{ fontSize: 15, fontWeight: 400, color: sky.txt, letterSpacing: '-0.005em' }}>{c.label}</div>
            ...
            <div style={{ marginTop: 20, animation: 'varaFadeUp 0.5s ease' }}>
                ...
                border: '0.5px solid ' + line, borderLeft: '2px solid ' + chip.color
                  ...
                  <p style={{
                  fontSize: 14.5, fontWeight: 300, color: sky.sub,
                    ...
                    return (
                      <button key={ex.id} onClick={function () {onOpenPractice(ex);}} style={{
                        ...
                        cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--vara-sans)'
                          ...
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontSize: 16, color: chip.color
                        }}>{TYPE_GLYPH[ex.type]}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 16, fontWeight: 400, color: sky.txt, letterSpacing: '-0.005em' }}>{ex.name}</div>
                            <div style={{ fontSize: 11.5, fontWeight: 300, color: sky.sub, marginTop: 3, letterSpacing: '0.04em' }}>{ex.minutes} min · {ex.subtitle}</div>
                          </div>
          ...
          <div style={{ padding: '64px 24px 0' }}>
              ...
              {(PRACTICES[rhythmTab] || []).map(function (p) {
                ...
                return (
                  <button key={p.id} onClick={function () {onOpenPractice(p);}} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 16, fontWeight: 400, color: sky.txt, letterSpacing: '-0.005em' }}>{p.name}</div>
                      <div style={{ fontSize: 9.5, fontWeight: 300, color: sky.sub, marginTop: 2, letterSpacing: '0.04em' }}>{p.minutes} min \u00B7 {p.subtitle}</div>
                    </div>
                    <span style={{ fontSize: 16, color: tabColor, opacity: 0.7, flexShrink: 0 }}>{'\u2192'}</span>
                  </button>);
            ...
            <button onClick={function () {onNavigate('library');}} style={{
              ...
              background: 'transparent', border: 'none', cursor: 'pointer',
              fontFamily: 'var(--vara-sans)',
              fontSize: 12.5, fontWeight: 400, letterSpacing: '0.06em', color: sky.sub,
              textAlign: 'center'
          ...
          <div style={{ padding: '70px 28px 0' }}>
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 400, color: sky.txt, letterSpacing: '-0.005em' }}>Day 1 of 7</div>
                <div style={{ fontSize: 12.5, fontWeight: 300, color: sky.sub, marginTop: 4, letterSpacing: '0.02em', lineHeight: 1.5 }}>Each morning scan sharpens your baseline.</div>
              </div>
