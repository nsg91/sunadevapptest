...
// State-chip → curated Library practices (mapped to the master protocol)
// Each chip pulls 3 practices from PRACTICES so Home and Library share one source of truth.
const CHIP_PRACTICES = {
  Reactive: ['physioSigh', 'vagalHum', 'somaticShake'],
  Anxious:  ['physioSigh', 'butterflyHug', 'coherentWave'],
  Wired:    ['heartHold', 'physioSigh', 'frontalBridge'],
  Open:     ['coherentWave', 'softGaze', 'heartHold'],
  Dimmed:   ['saccadicReset', 'auricularReset', 'coldPoint'],
  Stuck:    ['somaticShake', 'danceLikeNoOne', 'curiosityScan'],
};
function getChipPractices(chipLabel) {
  var ids = CHIP_PRACTICES[chipLabel] || [];
  return ids
    .map(function(id) { return ALL_PRACTICES.find(function(p) { return p.id === id; }); })
    .filter(Boolean);
}
...
Object.assign(window, {
  PRACTICES, ALL_PRACTICES, CHIP_PRACTICES, getChipPractices,
