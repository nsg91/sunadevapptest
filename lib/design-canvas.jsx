...
function DesignCanvas({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      // Let the event pass through natively if the cursor is over an
      // opted-in passthrough subtree (e.g. a phone screen with its own
      // inner scroll). ctrl+wheel (pinch zoom) still counts for us.
      if (!e.ctrlKey) {
        const pt = e.target && e.target.closest && e.target.closest('[data-dc-passthrough]');
        if (pt) return; // don't preventDefault → native scroll wins
      }
      e.preventDefault();
